...
class FVSEOF(Simulator.Simulator):
    ...
    def run_FVSEOF(self, biomass_rxn, target_rxn, flux_constraints={}):
        ...
        cnt = 1
        flux_info = {}
        for each_target_flux in tqdm.tqdm(manipulation_ratio_list):
            flux_const = {}
            ...
            for each_rxn in tqdm.tqdm(all_reactions):                
                a,b,c = self.run_FBA(new_objective=each_rxn, flux_constraints=flux_const2, mode='min')
                min_value = c[each_rxn]

                a,b,c = self.run_FBA(new_objective=each_rxn, flux_constraints=flux_const2, mode='max')
