...
class Single_Obj_GA(Simulator.Simulator):
    ...
    def set_reference_flux(self, filename):
        print 'Set reference flux'
        self.ref_flag = True
        self.ref_flux = {}
        fp = open(filename, 'r')
        fp.readline()
        for line in fp:
            sptlist = line.split('\t')
            self.ref_flux[sptlist[0].strip()] = float(sptlist[1].strip())
        fp.close()
        return
    ...
    def calc_wild_dist(self):
        if self.ref_flag == False:
            ...
            if self.experimental_flux_data == {}:
                ...
                WildGrowth = FluxDist[self.GrowthRxn]
                ...
                model_stat, objective_value, fitted_fluxdist = self.run_MOMA(wild_flux=fba_fluxdist, flux_constraints=constraint_info)
            ...
            print 'Use reference flux'
            self.wild_flux_dist = self.ref_flux
            WildGrowth = self.ref_flux[self.GrowthRxn]
            WildTargetProduction = self.ref_flux[self.target_reaction]
    ...
    def mutate_gene(self, individual, indpb=0.05):
        import random
        size = len(individual)
        for i in xrange(size):
            ...
            prop = random.random()
            if prop < indpb:
                individual[i] = random.choice(list(set(self.kd_ratio_list).difference(set([original_value]))))
        return individual
...
if __name__ == '__main__':
    ...
    obj.set_gene_manipulation_list(
        [0.0, 1.0])  # For KO simulation : [0.0, 1.0] , For KD & KO simulation : [0.0, 0.5, 1.0]
    ...
    obj.save_result('20160604_single_optgene')
